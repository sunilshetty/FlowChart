(function () {
		...
		this.compile = function (a, b) {
			...
			if (c) {
				for (var d = c.getFunctionBody(this), e = a.customTags[this.tag] ? "_rotors.customTags['" + this.tag + "'].rendered(_le, _rotors);" : "_rotors.globalTags['" + this.tag + "'].rendered(_le, _rotors);", f = 0; f < this.children.length; f++)
					this.children[f].precompile && (d += this.children[f].precompile(a)), d += this.children[f].compile(a), this.children[f].postcompile && (d += this.children[f].postcompile(a));
			...
			for (var j = 0; j < this.children.length; j++)
				this.children[j].precompile && (g += this.children[j].precompile(a)), g += this.children[j].compile(a), this.children[j].postcompile && (g += this.children[j].postcompile(a));
			return this.remove === !0 || b || (g += "_le=_els.pop();", g += "_rotors.pet(_eid, '" + this.uuid + "');"),
			g
		};
		var e = function (a, c) {
	...
	C = function (a) {
		...
		this.templateResolver = a.templateResolver || A(this)
	},
	D = function (a, b) {
		for (var c in b)
	...
	D(C.prototype, {
		...
		customElements : {
			...
			"r-tmpl" : {
				...
				parse : function (a, b, c, d, e, f) {
					if (a.type = "template", a.context = a.atts.context, a.atts.lookup)
						a.lookup = a.atts.lookup, a["default"] = a.atts["default"] || "", a.compile = function (b) {
		...
		parse : function (a, b, c, d) {
			...
			q = function (a, b, c, d, e) {
				...
				return h && (h.parse(g, b, c, d, f, e), h.compile && (g.compile = h.compile), g.precompile = h.precompile, g.postcompile = h.postcompile, g.custom = !0, g.remove = h.remove, d.debug("  element is a custom element"), d.maybeDebug(g.remove, "  element's root should not appear in output")),
				g
			},
			r = [{
					re : h.commentRe,
			...
			f = function (a) {
				if (a.nodeType === Node.ELEMENT_NODE) {
					for (var h = 0, i = a.attributes.length; i > h; h++)
						c = a.attributes[h], "rotors" === c.name ? (d = c.value, a.innerHTML = g(b, d)) : 0 === c.name.indexOf("rotors-") && (d = c.value, e = c.name.substring(7), a.setAttribute(e, g(b, d)));
...
.call("undefined" != typeof window ? window : this), true, function () {
	...
	j = (b.Graph = function (a) {
		...
		this.addEdge = function (a, b, c) {
			var d = null == a.directed ? j === !0 : !(a.directed === !1),
			g = a.cost || k,
			l = e(a.data, b, this),
			m = r(a.source, !0),
			n = r(a.target, !0);
			...
			return o.source.addEdge(o),
			o.source !== o.target && o.target.addEdge(o),
			f[l] = o,
			h++,
			o
		},
	...
	m = {
		...
		compute : function (a) {
			...
			for (b = 0; f > b; b++) {
				...
				for (g[i.id] || (g[i.id] = {}), h[i.id] || (h[i.id] = {}), g[i.id][i.id] = 0, c = 0; f > c; c++)
					if (b != c) {
...
.call("undefined" != typeof window ? window : this), true, function () {
	...
	a.jsPlumbToolkitInstance = function (h) {
		...
		B = h.saveUrl,
		C = h.onAutoSaveSuccess || function () {},
		D = h.onAutoSaveError || function () {},
		...
		this.addFactoryNode = function (a, b, c) {
			b = 2 != arguments.length || null != arguments[1] && "object" != typeof arguments[1] ? {}
			 : arguments[1],
		...
		this.addEdge = function (b, c, d) {
			var e = G.addEdge(b, n, this.beforeConnect);
			...
			g = function (d, e, g, h) {
				if (null != d)
					for (var i = d[b]({
						...
						l = g && d == G || i[j].target == d || c && i[j].target.constructor == jsPlumbGraph.Port && i[j].target.getNode() == d;
						(e && k || g && l || h && (k || l)) && f(i[j])
					}
			};
...
.call("undefined" != typeof window ? window : this), true, function () {
	...
	b = function (a) {
		...
		this.ensureNotEmpty = function () {j
			0 >= b && e()
		}
	},
	c = [],
	...
	c.Circular = function (a) {
		...
		this.step = function (a, c) {
			...
			for (e = 0; d > e; e++)
				...
				b.setPosition(f.id, g + Math.sin(k) * i, h + Math.cos(k) * i, !0), k += j;
...
.call("undefined" != typeof window ? window : this), true, function () {
	...
	c.Hierarchical = function (a) {
		...
		i,
		j,
		k = c.AbstractHierarchicalLayout.apply(this, arguments),
		l = [],
...
.call("undefined" != typeof window ? window : this), true, function () {
	...
	c.ActiveDragFilter = function (a, b, c) {
		...
		b.bind("connectionDrag", function (b) {
			...
			m = c.getNodeCount();
			if (k)
				for (e(k, l), g = l.getPorts(), h = 0; h < g.length; h++)
					e(k, g[h]);
			for (h = 0; m > h; h++) {
				var n = c.getNodeAt(h);
				for (e(k, n), g = n.getPorts(), i = 0; i < g.length; i++)
					e(k, g[i])
			}
		}),
...
.call(this), true, function () {
	...
	q = b.AbstractRenderer = function (b) {
		...
		b.activeFiltering && new c.UI.ActiveDragFilter(i, B, l),
		...
		if (this.bindToolkitEvents !== !1) {
			...
			l.bind(j.portAdded, function (a) {
				...
				ja(B.getElement(c), a.node, a.node.id),
				i.fire(j.portAdded, {
					node : a.node,
					nodeEl : b,
					port : a.port,
		...
		da = function (a, b, c, d, e, f, g, h) {
			return function (i, j, k) {
				...
				o.jtk[a] = i,
				o.jtk.node = k,
				f && r && ma.makeDraggable && ma.makeDraggable(o, q.dragOptions),
				s && ma.makeDroppable && ma.makeDroppable(o, q.dropOptions);
				var y = function (a) {
					B.on(o, a, function (b) {
						q.events[a]({
							node : k,
		...
		this.getRenderedElement = function (a) {
			if (null == a)
				return null;
			var b = a.getFullId();
			return "Port" === a.objectType ? L[b] : J[b]
		},
		...
		this.getRenderedEndpoint = function (a) {
			var b = la(a),
			c = null;
			return b && b.obj && "Port" === b.obj.objectType && B.selectEndpoints({
				element : b.el
			}).each(function (a) {
				...
				for (e = 0; e < a.childNodes.length; e++)
					if (3 != a.childNodes[e].nodeType && 8 != a.childNodes[e].nodeType) {
						...
						if (a.childNodes[e].tagName.toUpperCase() == g.SOURCE && null == a.childNodes[e].getAttribute("jtk-processed")) {
							...
							if (null != f.portId && (L[c + "." + f.portId] = a, a.jtk = a.jtk || {}, a.jtk.port = l.addPort(b, {
											id : f.portId
										}, !0)), m) {
							...
							var s,
							t = a._katavorioDrop ? a._katavorioDrop.length : 0;
							B.makeSource(a, f);
						...
						if (a.childNodes[e].tagName.toUpperCase() == g.TARGET && null == a.childNodes[e].getAttribute("jtk-processed")) {
							f = ia(a.childNodes[e], b, c),
							0 != d && (L[c + "." + f.portId] = a, a.jtk = a.jtk || {}, a.jtk.port = l.addPort(b, {
										id : f.portId
									}, !0)),
							B.makeTarget(a, f);
		...
		var la = function (a) {
			...
			l.getObjectInfo(a, function (a) {
				return a.getNode ? L[a.getFullId()] || J[a.getNode().id] : J[a.id]
			})
		};
	...
	c.Surface = function (a) {
		...
		C = new b.Widgets.Lasso({
				...
				onSelect : function (a, b, c, d) {
					...
					for (var g = 0; g < f.length; g++)
						(null == A || A(f[g].el.jtk.node) !== !1) && (e.push(f[g].el.jtk.node), m.jsPlumb.addToDragSelection && m.jsPlumb.addToDragSelection(f[g].el));
					z = e,
		...
		var S = function (a, b, c) {
			...
			if (d.el && ("Port" === !d.obj.objectType || !function () {
					...
					return a ? (e = a.setEnabled(c), !0) : void 0
				}
					())) {
				var f = "set" + a + "Enabled";
...
function () {
	...
	c.Miniview = function (a) {
		...
		u && (v = jsPlumb.createElement("div"), v.className = f.MINIVIEW_COLLAPSE, p.appendChild(v), e.on(v, g.click, function (a) {
				w = !w,
